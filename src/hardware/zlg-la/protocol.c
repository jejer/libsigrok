...
SR_PRIV const struct zlg_profile zlg_profiles[] = {
	{ ZLG_LA8,  "LA8",  "ChronoVu LA8",  8,  SR_MHZ(100), 2, 0.8388608 },
	{ ZLG_LA16, "LA16", "ChronoVu LA16", 16, SR_MHZ(200), 4, 0.042 },
	ALL_ZERO
};

/* LA8: channels are numbered 0-7. LA16: channels are numbered 0-15. */
SR_PRIV const char *zlg_channel_names[] = {
	"0", "1", "2", "3", "4", "5", "6", "7",
			       ...
			       uint64_t samplerate)
{
	...
	zlg_fill_samplerates_if_needed(sdi);
 ...
 */
static int zlg_read(struct dev_context *devc, uint8_t *buf, int size)
{
	int bytes_read;
...
static int close_usb_reset_sequencer(struct dev_context *devc)
{
	...
	if (devc->ftdic->usb_dev) {
		...
		(void) zlg_write(devc, buf, 8); /* Ignore errors. */
		g_usleep(100 * 1000);
...
static int reset_device(struct dev_context *devc)
{
	...
	do {
		...
		bytes_read = zlg_read(devc, (uint8_t *)&buf, BS);
...
SR_PRIV int zlg_read_block(struct dev_context *devc)
{
	...
	bytes_read = zlg_read(devc, devc->mangled_buf, BS);

	/* If first block read got 0 bytes, retry until success or timeout. */
	if ((bytes_read == 0) && (devc->block_counter == 0)) {
		do {
			sr_spew("Reading block 0 (again).");
			/* Note: If bytes_read < 0 zlg_read() will log errors. */
			bytes_read = zlg_read(devc, devc->mangled_buf, BS);
			now = g_get_monotonic_time();
