...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (i = 0; devlist[i]; i++) {
		...
		if (!strcmp(product, "zlg LA8"))
			model = 0;
		else if (!strcmp(product, "zlg LA16"))
			model = 1;
		else
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	switch (key) {
	case SR_CONF_SAMPLERATE:
		if (zlg_set_samplerate(sdi, g_variant_get_uint64(data)) < 0)
			return SR_ERR;
		break;
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_LIMIT_SAMPLES:
		if (!devc || !devc->prof)
			return SR_ERR_BUG;
		*data = std_gvar_tuple_u64(0, (devc->prof->model == ZLG_LA8) ? MAX_NUM_SAMPLES : MAX_NUM_SAMPLES / 2);
		break;
...
static int receive_data(int fd, int revents, void *cb_data)
{
	...
	for (i = 0; i < NUM_BLOCKS; i++)
		zlg_send_block_to_session_bus(sdi, i);
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	devc->divcount = zlg_samplerate_to_divcount(sdi, devc->cur_samplerate);
	if (devc->divcount == 0xff) {
		sr_err("Invalid divcount/samplerate.");
		return SR_ERR;
	}

	if (zlg_convert_trigger(sdi) != SR_OK) {
	...
	/* Start acquisition. */
	bytes_written = zlg_write(devc, buf, bytes_to_write);
